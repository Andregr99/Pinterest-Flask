...
from fakepinterest.forms import FormLogin, FormCriarConta, FormFoto, FormPesquisa
...
def pesquisar():
    ...
    try:
        ...
        if not resultados:
            flash("Nenhum resultado encontrado.", "info")  # Agora flash está definido
            return redirect(url_for('feed'))
        else:
            return render_template('resultados.html', resultados=resultados, query=query)
    except Exception as e:
        flash('Erro ao realizar a pesquisa. Tente novamente mais tarde.', 'danger')  # Agora flash está definido
        return redirect(url_for('feed'))
